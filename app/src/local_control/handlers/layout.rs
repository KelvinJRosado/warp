...
use ::local_control::protocol::{
    ...
    TabRenameParams, TabTarget, TargetSelector, WindowTarget,
};
...
pub(crate) fn rename_tab(
    ...
    let tab_id = entry.pane_group.id().to_string();
    let window_id_str = entry.window_id.to_string();
...
pub(crate) fn validate_tab_rename_target(target: &TargetSelector) -> Result<(), ControlError> {
