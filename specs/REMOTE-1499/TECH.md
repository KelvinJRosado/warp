...
The feature is composed of four orthogonal sub-systems on the warp-4 side, all shipping unconditionally on Stage 2 (no feature flag):
...
### 2.1 Feature flag
Stage 2 ships unconditionally. There is no feature flag — the three entry points are gated only by `OzHandoff && HandoffLocalCloud` and the source-content guardrail.
...
When the guardrail passes, all three entry points converge in `start_local_to_cloud_handoff` (`app/src/workspace/view.rs`), which synthesizes an empty `PendingCloudLaunch` for `launch: None` dispatches from chip/`&`/`/handoff` and collects attachments from the source input.
- `app/src/ai/blocklist/agent_view/agent_input_footer/mod.rs` `OpenHandoffPane` action: when the guardrail passes, dispatches `WorkspaceAction::OpenLocalToCloudHandoffPane { launch: None, environment_id: None, entry_point: HandoffEntryPoint::FooterChip }` directly. The chip skips `&` compose mode. **Fallback (guardrail fails):** emits the legacy `OpenHandoffPane` event so the source input enters `&` compose mode.
- `app/src/terminal/input.rs`: the empty-prompt early-return in `maybe_launch_cloud_handoff_request` keys on `source_conversation_has_content`; when the guardrail passes, it dispatches `OpenLocalToCloudHandoffPane { launch: None, ... }` and lets the workspace synthesizer construct the empty launch. **Fallback:** swallows the Enter (no-op) so the compose draft is preserved.
- `app/src/terminal/input/slash_commands/mod.rs` `/handoff` with no argument: when the guardrail passes, dispatches the same `OpenLocalToCloudHandoffPane { launch: None, ... }` as the chip. **Fallback (guardrail fails):** surfaces a `"Nothing to hand off — start a conversation first."` toast (no compose-mode activation).
